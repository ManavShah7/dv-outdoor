-- DV Outdoor Advertising — complete schema, consolidated into one file.
-- Safe to run on a brand-new Supabase project OR re-run on top of an
-- existing one (everything is dropped and recreated first). Your Supabase
-- Auth login (auth.users) is never touched — the backfill at the very
-- bottom re-attaches an admin profile to any existing auth user
-- automatically, so you won't be locked out after re-running this.
--
